 ...
 * - Coordinate with HighlightManager and TaskMapper
...
export class CrossViewHighlightManager {
    constructor(domRegistry = null) {
        ...
        console.log('[CrossViewHighlightManager] Initialized');
    }
    ...
    setTaskMapper(taskMapper) {
        ...
        console.log('[CrossViewHighlightManager] TaskMapper set');
    }
    ...
    registerSection(sectionId, container) {
        ...
        console.log(`[CrossViewHighlightManager] Registered section: ${sectionId}`);
    }
    ...
    onTaskClicked(taskId, sourceFormat, sectionId) {
        console.log(`[CrossViewHighlightManager] Task clicked: ${taskId} in ${sectionId}`);
        ...
        if (taskId.includes(':task:')) {
            ...
            if (match) {
                baseTaskId = match[1];
                console.log(`[CrossViewHighlightManager] Extracted base ID: ${baseTaskId} from ${taskId}`);
            }
        }
    ...
    highlightWithTaskMapper(baseTaskId, sourceFormat, sectionId, originalTaskId) {
        console.log(`[CrossViewHighlightManager] Using TaskMapper to find related tasks for: ${baseTaskId} (original: ${originalTaskId})`);
        console.log(`[CrossViewHighlightManager] Source format: ${sourceFormat}, Section ID: ${sectionId}`);
        ...
        if (equivalentTasks.length === 0) {
            console.log(`[CrossViewHighlightManager] No equivalent tasks found for: ${baseTaskId}`);
            console.log(`[CrossViewHighlightManager] Just highlighting in source section: ${sectionId}`);
            // Just highlight in source section using original ID
            this.highlightInSection(sectionId, originalTaskId, true);
            return;
        }
        
        console.log(`[CrossViewHighlightManager] Found ${equivalentTasks.length} equivalent tasks`);
        ...
        equivalentTasks.forEach(({ taskId: mappedTaskId, format: mappedFormat, confidence }) => {
            ...
            console.log(`[CrossViewHighlightManager] Highlighting ${mappedTaskId} in ${mappedSectionId} (isActive=${isActive}, confidence=${confidence})`);
    ...
    findEquivalentTasks(taskId, sourceFormat) {
        if (!this.currentStepMapping) {
            console.log('[CrossViewHighlightManager] No task mapping available for current step');
            return [];
        }
        
        console.log(`[CrossViewHighlightManager] Finding equivalent tasks for ${taskId} in ${sourceFormat}`);
        ...
        try {
            ...
            console.log(`[CrossViewHighlightManager] Task mapping returned:`, equivalents);
            
            // Convert to array format
            const result = [];
            Object.entries(equivalents).forEach(([format, tasks]) => {
                console.log(`[CrossViewHighlightManager] Format ${format} has ${tasks.length} tasks`);
                tasks.forEach(({ task, confidence }) => {
                    ...
                    console.log(`[CrossViewHighlightManager]   → ${task.id} (${task.label}) in ${format}`);
                });
            });
            
            console.log(`[CrossViewHighlightManager] Found ${result.length} equivalent tasks`);
            return result;
            
        } catch (error) {
            console.error('[CrossViewHighlightManager] Error finding equivalent tasks:', error);
            return [];
        }
    }
     ...
     */
    highlightInSection(sectionId, taskId, isActive = false, confidence = 1.0) {
        const container = this.sections[sectionId];
        if (!container) {
            console.log(`[CrossViewHighlightManager] Section not found: ${sectionId}`);
            return;
        }
        
        // Check if section is in visual mode (not raw mode)
        if (!this.isSectionInVisualMode(sectionId)) {
            console.log(`[CrossViewHighlightManager] Section ${sectionId} is in raw mode, skipping highlight`);
            return;
        }
        ...
        if (!taskElement) {
            console.log(`[CrossViewHighlightManager] Task element not found: ${taskId} in ${sectionId}`);
            return;
        }
        ...
        // Apply highlighting
        if (isCpeeSection) {
            this.highlightManager.highlightCPEETask(taskElement, isActive);
        } else if (isMermaidSection) {
            this.highlightManager.highlightMermaidNode(taskElement, isActive);
        } else {
            this.highlightManager.highlightElements([taskElement], isActive);
        }
        ...
        console.log(`[CrossViewHighlightManager] Highlighted ${taskId} in ${sectionId} (isActive=${isActive})`);
        console.log(`[CrossViewHighlightManager] Task element classes after highlight:`, taskElement.className);
    }
    ...
    findTaskInSVG(container, taskId) {
        console.log(`[CrossViewHighlightManager] Searching for task: ${taskId} in container`);
        
        // First, try CPEE element-id attribute (most reliable for CPEE)
        const elements = container.querySelectorAll('[element-id]');
        console.log(`[CrossViewHighlightManager] Found ${elements.length} elements with element-id attribute`);
        for (const el of elements) {
            const elementId = el.getAttribute('element-id');
            if (elementId === taskId) {
                console.log(`[CrossViewHighlightManager] ✓ Found CPEE task by element-id: ${taskId}`);
                return el;
            }
        }
        ...
        console.log(`[CrossViewHighlightManager] Found ${nodes.length} Mermaid nodes`);
        
        // Try exact ID match for Mermaid
        for (const node of nodes) {
            if (node.id === taskId) {
                console.log(`[CrossViewHighlightManager] ✓ Found Mermaid node by exact ID: ${taskId}`);
                return node;
            }
        }
        ...
        for (const node of nodes) {
            if (node.id && node.id.includes(taskId)) {
                console.log(`[CrossViewHighlightManager] ✓ Found Mermaid node by partial ID: ${node.id}`);
                return node;
            }
        }
        ...
        if (baseIdMatch) {
            ...
            console.log(`[CrossViewHighlightManager] Extracted base ID: ${baseId}`);
            for (const node of nodes) {
                if (node.id && node.id.includes(baseId)) {
                    console.log(`[CrossViewHighlightManager] ✓ Found Mermaid node by base ID: ${node.id}`);
                    return node;
                }
            }
        }
        ...
        try {
            ...
            if (element) {
                console.log(`[CrossViewHighlightManager] ✓ Found task by ID: ${taskId}`);
                return element;
            }
        } catch (e) {
            console.log(`[CrossViewHighlightManager] ID selector failed: ${e.message}`);
        }
        ...
        try {
            ...
            if (element) {
                console.log(`[CrossViewHighlightManager] ✓ Found task by data-task-id: ${taskId}`);
                return element;
            }
        } catch (e) {
            console.log(`[CrossViewHighlightManager] data-task-id selector failed`);
        }
        
        console.log(`[CrossViewHighlightManager] ✗ Could not find task element: ${taskId} in container`);
        return null;
    }
    ...
    isSectionInVisualMode(sectionId) {
        ...
        // This would integrate with ViewModeManager
    ...
    clearAllHighlights() {
        ...
        console.log('[CrossViewHighlightManager] Cleared all highlights');
    }
    ...
    onStepChanged() {
        ...
        this.activeSourceFormat = null;
        this.activeSourceSection = null;
        console.log('[CrossViewHighlightManager] Step changed, cleared highlights');
    }
    ...
    refreshSections() {
        if (!this.domRegistry) {
            console.log('[CrossViewHighlightManager] No DOMRegistry available');
            return;
        }
        ...
        sectionIds.forEach(sectionId => {
            ...
            if (container) {
                this.sections[sectionId] = container;
                console.log(`[CrossViewHighlightManager] Refreshed section: ${sectionId}`);
            } else {
    ...
    reset() {
        ...
        this.highlightedTasks.clear();
        console.log('[CrossViewHighlightManager] Reset');
    }
}
