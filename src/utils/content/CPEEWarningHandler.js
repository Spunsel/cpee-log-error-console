/**
 * CPEEWarningHandler - Handles preprocessing warnings for CPEE XML rendering
 * 
 * Displays warning panels when preprocessing steps have been applied to CPEE XML.
...
export class CPEEWarningHandler {
    /**
    ...
    static displayWarningIndicator(container, appliedSteps) {
        if (!container) {
            console.error('CPEEWarningHandler: Cannot display warning - container is null');
            return;
        }
        ...
        warningBox.className = 'cpee-warning-indicator';

        // Create warning icon
        const warningIcon = document.createElement('div');
        warningIcon.className = 'cpee-warning-indicator__icon';
        warningIcon.innerHTML = ICON_WARNING;

        // Create expand/collapse button
        const toggleButton = document.createElement('button');
        toggleButton.className = 'cpee-warning-indicator__toggle';
        ...
        stepsListContainer.className = 'cpee-warning-indicator__steps-container';
        stepsListContainer.style.display = 'none'; // Default to hidden

        // Create steps list
        const stepsList = document.createElement('ul');
        stepsList.className = 'cpee-warning-indicator__steps';
        appliedSteps.forEach(step => {
            ...
            if (typeof step === 'string') {
                ...
                if (step.lineNumbers && step.lineNumbers.length > 0) {
                    ...
                    lineNumbersSpan.className = 'cpee-warning-indicator__line-numbers';
        ...
        warningHeader.className = 'cpee-warning-indicator__header';
    ...
    static removeWarningIndicator(container) {
        ...
        const existingWarning = container.querySelector('.cpee-warning-indicator');
        if (existingWarning) {
